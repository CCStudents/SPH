...
#define StepN   3000         //ステップ数
#define Rho1    1.0         //初期密度1
#define Rho2    1.0        //初期密度2
#define Vel1    0         //初期速度1
#define Vel2    0         //初期速度2
#define Press1  1000         //初期圧力1
#define Press2  0.01      //初期圧力2
#define Alpha   2.0         //粘性項のα
#define Beta    4.0         //粘性項のβ
...
void InicialCondi  ( double m[], double x[], double v[], double a[], double p[], double d[], double u[], double du[], double h[] )
{
  ...
  for(i = 0; i < N_ALL; i++){
    for(j = 0; j < N_ALL; j++){
      ...
      //du[l]= du[l] + (p[n] / d[n] / d[n] ) * (v[l] - v[n] ) / 2 * DifferKernelFunc(l, n, x, h);
    }
  }
}
