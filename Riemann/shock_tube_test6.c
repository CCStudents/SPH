...
#define TIMEOUT	0.005		//出力時間
#define dt		0.0005		//タイムステップ
...
void SAMPLE(double S, double WS[4], double W[4], double WL[4], double WR[4], double G[4]){
	double SHL,STL;		//Left rarefaction head,tailの速度
	double SHR,STR;		//Right rarefaction head,tailの速度
	double CML,CMR;		//rarefaction内の加速度の計算(a_*L,a*R)
...
//					printf("check:left rarefaction in tail.\n");
					...
					//ここのγの値に注意
					W[0] = WL[0]*pow((WS[2]/WL[2]),(1.0/1.66667));
					W[1] = WS[1];
					W[2] = WS[2];
				}else{
...
*/	
	...
	for(i=1; i<=CELLS; i++){
		XPOS = ((double)i-0.5)*DX;	//x座標の計算(何個目のbinか)
		S = (XPOS-DIAPH)/TIMEOUT;	//そのxでのTIMEOUT時における音速の計算
